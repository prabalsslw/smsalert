...
class Smsalert{
    ...
    private $url='http://sms.sslwireless.com/pushapi/dynamic/server.php';   // Define url
    private $urlplus='https://smsplus.sslwireless.com/api/v3/send-sms';   // Define url
    ...
    *****************************************************************************************/
    ...
    {
        $this->apitoken = array('api_token'=>$apitoken);;
        return $this;
    }
    ...
    *****************************************************************************************/
    ...
    {
        $this->authParams = array('user'=>$user,'pass'=>$pwd);
        return $this;
    }
    ...
    *****************************************************************************************/
    ...
    private function getApiToken()
    {
        ...
        {
            die('Please Enter Correct API Hash Token for ISMS Plus');
        }else{
    ...
    *****************************************************************************************/
    ...
    {   
        ...
        if($this->platform == "ISMS")
        {
            $params = array('sid'=>$this->sender,'sms'=>$text, 'msisdn'=>$this->formatNumber($mobileno), 'csmsid'=> uniqid());
            $params   = array_merge($params,$this->getAuthParams());
     
            $client   = new Client();
            $response = $client->request('GET', $url, ['query' => $params, 'http_errors' => false]);
            $body     = json_decode($response->getBody(),TRUE); 
        }
        elseif($this->platform == "ISMSPLUS")
        {
            $params = array('sid'=>$this->sender,'sms'=>$text, 'msisdn'=>$this->formatNumber($mobileno), 'csms_id'=> uniqid());
            $params   = array_merge($params,$this->getApiToken());
       
            $client   = new Client();
            $response = $client->request('GET', $urlplus, ['query' => $params], ['Content-Type: application/json','Content-Length: ' . strlen($params),'accept:application/json']);
            $body     = json_decode($response->getBody(),TRUE); 
        }
        return $body;
    }
